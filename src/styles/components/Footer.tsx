...
// Hapus impor 'Facebook', biarkan ikon yang tersisa
import { Mail, Phone, MapPin, Instagram } from 'lucide-react'; 
import { TikTok_Sans } from 'next/font/google';

// Komponen SVG Ikon TikTok
// Menggunakan komponen terpisah agar lebih bersih
...
const Footer = () => {
  return (
    // Gunakan warna kontras tipikal (misalnya, abu-abu gelap) untuk membedakan dari body
    <footer className="bg-gray-800 text-white pt-12 pb-8">
      <div className="mx-auto max-w-7xl px-6">
        
        {/* Grid Footer - Dipisahkan menjadi 4 Kolom */}
        <div className="grid grid-cols-1 gap-8 md:grid-cols-4 md:gap-16">
          ...
          <div>
            <Link href="/" className="flex items-center mb-4">
              ...
              <span className="ml-2 text-xl font-bold text-white">AIKURO GAKUEN</span>
            </Link>
          ...
          {/* Kolom 2: Navigasi Cepat (Berdasarkan Konsolidasi Header) */}
          <div>
            <h3 className="text-lg font-semibold mb-4 text-white">Navigasi</h3>
            <ul className="space-y-2">
              <li><Link href="/program" target="_blank" className="text-sm text-gray-400 hover:text-red-400 transition-colors">Semua Program</Link></li>
              <li><Link href="/profil" target="_blank" className="text-sm text-gray-400 hover:text-red-400 transition-colors">Tentang Kami</Link></li>
              <li><Link href="https://jisannihon.vercel.app" className="text-sm text-gray-400 hover:text-red-400 transition-colors">Blog & Berita</Link></li>
              <li><Link href="/daftar" target="_blank" className="text-sm text-gray-400 hover:text-red-400 transition-colors">Pendaftaran</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="flex space-x-4">
              {/* Mengganti Facebook dengan TikTok */}
              <a 
                href="URL_AKUN_TIKTOK_ANDA" 
                aria-label="TikTok" 
                className="text-gray-400 hover:text-red-400 transition-colors"
                target="_blank" 
                rel="noopener noreferrer"
              >
                <TikTokIcon className="h-6 w-6" /> {/* Menggunakan komponen SVG custom */}
              </a>
              <a 
